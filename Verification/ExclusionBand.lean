...
/-- A measurable horizontal hole of fixed width. Degenerate parameter limits are allowed. -/
structure ExclusionBand where
  width : ℝ
  width_nonneg : 0 ≤ width
  width_lt_one : width < 1
  lower : I → ℝ
...
noncomputable def density (p : I × I) : ℝ :=
  if B.lower p.1 ≤ (p.2 : ℝ) ∧ (p.2 : ℝ) ≤ B.lower p.1+B.width then 0 else 1/(1-B.width)
...
theorem density_mem (p : I × I) : B.density p ∈ Icc 0 (1/(1-B.width)) := by
  have hb : 0 < 1-B.width := sub_pos.mpr B.width_lt_one
  unfold density
  split_ifs
...
theorem density_integrable : Integrable B.density := by
  refine (integrable_const (1/(1-B.width))).mono' B.density_measurable.aestronglyMeasurable (Eventually.of_forall fun p => ?_)
...
theorem density_row_integrable (u : I) : Integrable (fun v : I => B.density (u,v)) := by
  refine (integrable_const (1/(1-B.width))).mono' (B.density_measurable.comp (by fun_prop)).aestronglyMeasurable
...
theorem density_row_integral (u : I) : (∫ v : I, B.density (u,v))=1 := by
  let a : I := ⟨B.lower u,B.lower_nonneg u,by linarith [B.lower_le u,B.width_nonneg]⟩
  let b : I := ⟨B.lower u+B.width,by linarith [B.lower_nonneg u,B.width_nonneg],by linarith [B.lower_le u]⟩
  have hi : Integrable ((Icc a b).indicator (fun _ : I => (1 : ℝ))) := (integrable_const _).indicator measurableSet_Icc
  have he : (fun v : I => B.density (u,v))=fun v => (1/(1-B.width))*(1-(Icc a b).indicator (fun _ => (1 : ℝ)) v) := by
    funext v
    change (if B.lower u ≤ (v : ℝ) ∧ (v : ℝ) ≤ B.lower u+B.width then 0 else 1/(1-B.width))=_
    simp only [indicator,mem_Icc]
    change (if a ≤ v ∧ v ≤ b then _ else _)=(1/(1-B.width))*(1-if a ≤ v ∧ v ≤ b then 1 else 0)
    split_ifs <;> simp
  rw [he,integral_const_mul,integral_sub (integrable_const _) hi,
    integral_indicator measurableSet_Icc,setIntegral_one_eq_measureReal]
  simp only [integral_const,smul_eq_mul,mul_one,Measure.real,measure_univ,ENNReal.toReal_one,unitInterval.volume_Icc]
  have hab : (b : ℝ)-(a : ℝ)=B.width := by dsimp [a,b]; ring
  rw [hab,ENNReal.toReal_ofReal B.width_nonneg]
  exact div_mul_cancel₀ _ (sub_pos.mpr B.width_lt_one).ne'
...
/-- Standardize the second marginal of the uniform density outside the hole. -/
