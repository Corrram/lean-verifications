...
theorem tEV_one_pickands_half (ν : ℝ) (hν : 0<ν) :
  ...
  rw [integral_bivariateGaussian_mix ⟨by norm_num,le_rfl⟩
    ...
    (((hWm.comp measurable_fst).const_mul _).max ((hWm.comp measurable_snd).const_mul _))]
...
theorem tEV_one (ν : ℝ) (hν : 0<ν) :
  ...
  rw [extremalCoefficient_eq_twice_pickands _ (tEV_isExtremeValue ν 1 hν ⟨by norm_num,le_rfl⟩),
...
theorem tEVStableTail_negative_one (ν : ℝ) (hν : 0<ν) {x y : ℝ} (hx : 0≤x) (hy : 0≤y) :
  ...
  rw [integral_bivariateGaussian_mix ⟨le_rfl,by norm_num⟩
    (fun a b => max (x*gaussianPositiveWeight ν a) (y*gaussianPositiveWeight ν b))
    (((hWm.comp measurable_fst).const_mul _).max ((hWm.comp measurable_snd).const_mul _))]
  ...
  rw [integral_prod (fun p : ℝ×ℝ => x*gaussianPositiveWeight ν p.1+y*gaussianPositiveWeight ν (-p.1))
    ((((hWi.const_mul x).comp_fst (gaussianReal 0 1)).add
...
theorem tEV_negative_one (ν : ℝ) (hν : 0<ν) :
  ...
  intro u v
  unfold tEV
  rw [stableTailCopula_cdf,cdf_independence,Fin.prod_univ_two]
