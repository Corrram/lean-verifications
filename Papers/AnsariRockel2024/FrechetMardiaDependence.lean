...
theorem mardia_ci_iff (θ : ℝ) (hθ : |θ| ≤ 1) :
    (Copula.mardia θ hθ).IsCI ↔ θ = 0 ∨ θ = 1 :=
  Copula.mardia_ci_iff θ hθ

theorem mardia_cd_iff (θ : ℝ) (hθ : |θ| ≤ 1) :
    (Copula.mardia θ hθ).IsCD ↔ θ = 0 ∨ θ = -1 :=
  Copula.mardia_cd_iff θ hθ

theorem mardia_absolutelyContinuous_iff (θ : ℝ) (hθ : |θ| ≤ 1) :
    (Copula.mardia θ hθ).toMeasure ≪ (volume : Measure (Fin 2 → I)) ↔ θ = 0 :=
  Copula.mardia_absolutelyContinuous_iff θ hθ

theorem mardia_density_tp2_iff (θ : ℝ) (hθ : |θ| ≤ 1) :
    (Copula.mardia θ hθ).HasMTP2Density ↔ θ = 0 :=
  Copula.mardia_density_tp2_iff θ hθ

theorem mardia_not_lowerOrthant_ordered :
    ¬(Copula.mardia (1 / 2) (by norm_num)).LowerOrthantLE (Copula.mardia 0 (by norm_num)) ∧
      ¬(Copula.mardia 0 (by norm_num)).LowerOrthantLE (Copula.mardia (1 / 2) (by norm_num)) :=
  Copula.mardia_not_lowerOrthant_ordered
