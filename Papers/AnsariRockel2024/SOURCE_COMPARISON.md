...
The negative Student-t endpoint is now also proved: `student_negative_one` identifies it with W for every real nu>0, and `student_negative_one_isCD` proves CD there. Thus the closed parameter range includes both CI and CD singular members. The common marginal-law identity is verified independently of correlation; the analytic marginal and joint densities are now checked, while the interior conditional classifications remain unfinished.
