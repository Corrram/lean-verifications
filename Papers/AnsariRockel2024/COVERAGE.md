...
| Table 3: positive Clayton CI | `Papers.AnsariRockel2024.clayton_positive_ci` | verified | Every θ>0 is conditionally increasing in both directions on the closed square. The library proof establishes concavity of each positive-parameter CDF section and uses Archimedean symmetry. This verifies the CI part of the printed table cell; density TP2 remains open. |
...
| Table 3: Clayton parameter endpoints | `Papers.AnsariRockel2024.clayton_zero_ci_cd`; `Papers.AnsariRockel2024.clayton_negative_one_cd` | verified | At θ=0 the independence copula is both CI and CD. At θ=−1 the negative Clayton copula equals W and is CD. The interval −1<θ<0 remains open for CD. |
| Table 3: negative Clayton quadrant dependence | `Papers.AnsariRockel2024.clayton_negative_nqd` | verified | Every θ in [-1,0) has C(u,v)≤uv on the closed square, including W at θ=-1. This is a consequence expected from the printed CD entry, but does not verify CD itself. The proof lives in the pinned copula package. |
