...
| Remark 2.3: arbitrary SI blocks on an adjacent countable partition | `Papers.Rockel2026XiFootrule.ordinalSum_isSI_components`; `Papers.Rockel2026XiFootrule.countableOrdinal_isSI`; `Papers.Rockel2026XiFootrule.countableOrdinal_isSI_iff`; `Papers.Rockel2026XiFootrule.countableOrdinal_symmetric_si_rank_equality` | verified | A binary ordinal sum at an interior cut is SI only if both components are SI. For any adjacent countable positive-length partition exhausting [0,1], its ordinal sum is SI iff every component is SI. Symmetry and xi=footrule also pass from all components. Does not characterize all symmetric SI equality copulas. |
