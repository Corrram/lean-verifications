...
of the two Student-t terms is `-K t f(z_t) (σ²-2rσ+1)` with `K>0`, where the balance
identity `(1-t) f(z_{1-t}) = t σ² f(z_t)` relates the two Student-t densities.
-/
...
theorem tEVArg_eq_div (ν r σ : ℝ) (hν : 0<ν) :
    tEVArg ν r σ=Real.sqrt (1+ν)*((σ-r)/Real.sqrt (1-r^2)) := by
  unfold tEVArg
  rw [Real.sqrt_div (by linarith)]
  ring

/-- Derivative of the Pickands argument in the correlation parameter. -/
noncomputable def tEVArgDeriv (ν r σ : ℝ) : ℝ :=
  Real.sqrt (1+ν)*((r*σ-1)/((1-r^2)*Real.sqrt (1-r^2)))

theorem tEVArg_hasDerivAt (ν σ : ℝ) (hν : 0<ν) {r : ℝ} (hr : r∈Ioo (-1) 1) :
    HasDerivAt (fun q => tEVArg ν q σ) (tEVArgDeriv ν r σ) r := by
  ...
  have h3 := ((hasDerivAt_id r).const_sub σ).div h2 hsq.ne'
  have h4 := h3.const_mul (Real.sqrt (1+ν))
  have he : (fun q => tEVArg ν q σ)=fun q => Real.sqrt (1+ν)*((σ-id q)/Real.sqrt (1-q^2)) := by
    funext q
    rw [tEVArg_eq_div ν q σ hν]
    rfl
  rw [he]
  convert h4 using 1
  unfold tEVArgDeriv
...
theorem tEVArg_sq (ν r σ : ℝ) (hν : 0<ν) (hr : r∈Ioo (-1) 1) :
    (tEVArg ν r σ)^2=(1+ν)/(1-r^2)*(σ-r)^2 := by
  have hs : 0<1-r^2 := by nlinarith [hr.1,hr.2]
  unfold tEVArg
  rw [mul_pow,Real.sq_sqrt (by positivity)]

/-- Balance identity of the Student-t densities at the two Pickands arguments. -/
theorem tEV_density_balance (ν r σ : ℝ) (hν : 0<ν) (hr : r∈Ioo (-1) 1) (hσ : 0<σ) :
    studentMarginalPDF (ν+1) (tEVArg ν r σ⁻¹)=
      (σ^2)^((ν+1)/2+1/2)*studentMarginalPDF (ν+1) (tEVArg ν r σ) := by
  have hs : 0<1-r^2 := by nlinarith [hr.1,hr.2]
  have hX : 0<(ν+1)/2+(tEVArg ν r σ)^2/2 := by positivity
  have hσ2 : 0<σ^2 := by positivity
  have hbase : (ν+1)/2+(tEVArg ν r σ⁻¹)^2/2=((ν+1)/2+(tEVArg ν r σ)^2/2)/σ^2 := by
    rw [tEVArg_sq ν r _ hν hr,tEVArg_sq ν r _ hν hr]
    field_simp
    ring
  unfold studentMarginalPDF
  rw [hbase,Real.div_rpow hX.le hσ2.le]
  have hp : 0<(σ^2)^((ν+1)/2+1/2) := Real.rpow_pos_of_pos hσ2 _
  have hq : 0<((ν+1)/2+(tEVArg ν r σ)^2/2)^((ν+1)/2+1/2) := Real.rpow_pos_of_pos hX _
  field_simp
...
theorem tEVPickandsExpr_hasDerivAt (ν t : ℝ) (hν : 0<ν) (ht : t∈Ioo (0:ℝ) 1) {r : ℝ}
    (hr : r∈Ioo (-1) 1) :
...
theorem tEVPickandsExpr_deriv_nonpos (ν t : ℝ) (hν : 0<ν) (ht : t∈Ioo (0:ℝ) 1) {r : ℝ}
  ...
  set σ : ℝ := (t/(1-t))^(1/ν) with hσdef
  have hσ : 0<σ := Real.rpow_pos_of_pos hq _
  have hinv : ((1-t)/t)^(1/ν)=σ⁻¹ := by
    rw [hσdef,← Real.inv_rpow hq.le,inv_div]
  have hσν : σ^ν=t/(1-t) := by
    rw [hσdef,← Real.rpow_mul hq.le,one_div_mul_cancel hν.ne',Real.rpow_one]
  have hP : (1-t)*(σ^2)^((ν+1)/2+1/2)=t*σ^2 := by
    have : (σ^2)^((ν+1)/2+1/2)=σ^ν*σ^2 := by
      rw [← Real.rpow_natCast,← Real.rpow_mul hσ.le,← Real.rpow_natCast,← Real.rpow_add hσ]
      congr 1
      push_cast
      ring
    rw [this,hσν]
    field_simp
  rw [hinv,tEV_density_balance ν r σ hν hr hσ]
  have hs : 0<1-r^2 := by nlinarith [hr.1,hr.2]
  have hsq : 0<Real.sqrt (1-r^2) := Real.sqrt_pos.mpr hs
  set f := studentMarginalPDF (ν+1) (tEVArg ν r σ)
  ...
  have hD (τ : ℝ) : tEVArgDeriv ν r τ=K*(r*τ-1) := by
    ...
    ring
  rw [hD,hD]
  have he : (1-t)*((σ^2)^((ν+1)/2+1/2)*f*(K*(r*σ⁻¹-1)))+t*(f*(K*(r*σ-1)))=
      -(f*K*t*((σ-r)^2+(1-r^2))) := by
    have e1 : (1-t)*((σ^2)^((ν+1)/2+1/2)*f*(K*(r*σ⁻¹-1)))=
        ((1-t)*(σ^2)^((ν+1)/2+1/2))*f*(K*(r*σ⁻¹-1)) := by ring
    rw [e1,hP]
    field_simp
    ring
  rw [he]
  have : 0≤f*K*t*((σ-r)^2+(1-r^2)) := by
    have := ht.1
    positivity
  linarith
...
theorem tEVArg_one (ν r : ℝ) (hν : 0<ν) (hr : r∈Ioo (-1) 1) :
  ...
  unfold tEVArg
  rw [← Real.sqrt_sq h2.le,← Real.sqrt_mul (by positivity)]
  congr 1
  field_simp
  ring
...
theorem tEV_extremalCoefficient (ν r : ℝ) (hν : 0<ν) (hr : r∈Ioo (-1) 1) :
  ...
  rw [extremalCoefficient_eq_twice_pickands _ (tEV_isExtremeValue ν r hν _),tEV_pickands_half ν r hν hr]
